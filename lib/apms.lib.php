...
function apms_video_imgurl($url, $vid, $type) {
	...
	if($type == "file") { //JWPLAYER
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/v2.5/".$vid."?fields=id,picture&access_token=".$xp['facebook_token']);
	    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function apms_video($vid) {
	...
	if($video['type'] == "file") { //JWPLAYER
		...
		if($video['type'] == "youtube") { //유튜브
			...
			$autoplay = ($autoplayvideo || $video['auto']) ? '&autoPlay=1' : '';
			$show = '<iframe width="'.$video['width'].'" height="'.$video['height'].'" src="http://videofarm.daum.net/controller/video/viewer/Video.html?vid='.$video['vid'].'&play_loc=undefined'.$autoplay.'&wmode=opaque" frameborder="0" scrolling="no"'.$fullscreen.'></iframe>';
...
function apms_response($use, $flag, $it_id, $bo_table, $wr_id, $subject, $mb_id, $my_id='', $my_name='', $c_id='') {
	...
	if($is_update) {
		...
		//푸시 보내기
		$push = array('use'=>$use, 'flag'=>$flag, 'it_id'=>$it_id, 'bo_table'=>$bo_table, 'wr_id'=>$wr_id, 'my_id'=>$my_id, 'my_name'=>$my_name, 'c_id'=>$c_id);
...
function apms_script($name){
	...
	if($name == 'code') {
		...
		if(!defined('APMS_NIVOSLIDER')) {
			...
			add_stylesheet('<link rel="stylesheet" href="'.APMS_PLUGIN_URL.'/Nivo-Slider/nivo-slider.css" type="text/css" media="screen" />', 0);
...
function apms_multi_menu($menu, $id, $multi){
	...
	if($id && $multi) {
		...
		for ($i=0; $i < $cnt; $i++) {
			...
			$menu[$i]['multi'] = (isset($tmp[$i]['multi'])) ? $tmp[$i]['multi'] : '';
...
// SEO Keyword
function apms_seo_keyword($str1='', $str2='', $str3='') {
