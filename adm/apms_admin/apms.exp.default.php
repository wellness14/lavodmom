		...
		if($page <= $total_page) {
			...
			while($row=sql_fetch_array($result)) {
				...
				if($dxp) {
					...
					// 다운로드와 읽기를 제외한 모든 포인트 내역 삭제
					sql_query(" delete from {$g5['point_table']} where mb_id = '{$row['mb_id']}' and po_rel_action not in ('읽기', '다운로드') ", false);
					...
					// 경험치 합계
					$po_exp = $po_admin + $po_login + $po_chulsuk + $po_delivery + $po_write + $po_comment + $po_read + $po_good + $po_nogood + $po_download; 
